//i insert a gender and also addded a gender in the update user
...
export class UsersService {
  ...
  async createUser(username: string, password: string, gender: string, role = 'user') {
    const hashed = await bcrypt.hash(password, 10);
    const [result] = await this.pool().execute<OkPacket>(
      'INSERT INTO users (username, password, gender, role) VALUES (?, ?, ?, ?)',
      [username, hashed, gender, role],
    );
    return { id: result.insertId, username, gender, role };
  }
  ...
  async findById(id: number) {
    const [rows] = await this.pool().execute<RowDataPacket[]>(
      'SELECT id, username, password, gender, role, created_at FROM users WHERE id = ?',
      [id],
    );
    return rows[0];
  }
  ...
  async getAll() {
    const [rows] = await this.pool().execute<RowDataPacket[]>(
      'SELECT id, username, gender, role, created_at FROM users',
    );
    return rows;
  }
